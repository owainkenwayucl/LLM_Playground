...
# required 24-odd GiB RAM

# It does this by using the same process as the 2 GPU version, but instead of using a second GPU, does text processing
# on CPU. It also uses the fp4 quantisation for the image model.
...
text_model = "black-forest-labs/FLUX.2-dev"
platform = {"name": "Nvidia", "device":"cuda", "size":torch.bfloat16, "attention_slicing":False}

def setup_pipeline():
	text_encoder = Mistral3ForConditionalGeneration.from_pretrained(text_model, subfolder="text_encoder", torch_dtype=platform["size"])
	text_encoder_pipeline = Flux2Pipeline.from_pretrained(text_model, text_encoder=text_encoder, transformer=None, vae=None, torch_dtype=platform["size"])
