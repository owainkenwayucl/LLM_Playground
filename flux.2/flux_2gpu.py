...
def setup_pipeline():
	text_encoder = Mistral3ForConditionalGeneration.from_pretrained(model, subfolder="text_encoder", dtype=platform["size"]).to("cuda:1")
	...
	transformer = Flux2Transformer2DModel.from_pretrained(model, subfolder="transformer", dtype=platform["size"]).to("cuda:0")
	pipe = Flux2Pipeline.from_pretrained(model, text_encoder=None, tokenizer=None, transformer=transformer, dtype=platform["size"]).to("cuda:0")
