...
def process_daterange(config):
    ...
    for current_date in tqdm.tqdm(list(daterange_generator(start_date, stop_date))):
        date_timing = 0
        data[current_date.isoformat()] = {}

        for jobfile in tqdm.tqdm(list(os.scandir(jobscript_directory + current_date.isoformat()))):
